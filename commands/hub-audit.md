...
Spawn ONE **Sonnet** worker (multi-source judgment) with the prompt below. Give it NO framing beyond the task — it must not inherit the hub's assumptions. Its findings are **leads, not truths** (the hub's own rule); the main loop reviews each against reality before proposing anything to the operator.

**Worker prompt:**

> You are auditing a personal AI "hub" system for drift. Assume it IS drifting and find the evidence — a clean bill of health is a failure of imagination, but do NOT invent problems (every finding needs a cited file/line or ledger row). Read, read-only: `~/.claude/commands/hub.md` (the protocol), `~/.claude/hub/board.md`, `~/.claude/projects/<home-slug>/memory/MEMORY.md`, the last ~30 rows of `token_ledger.md`, and spot-check 3–5 memory files the index points to. Then report, terse and specific:
...
> 5. **Wrong optimization target** — anything the hub is busily optimizing that is NOT the operator's actual binding constraint (distribution / landing paying work). Is the machine helping him feel productive while the human outreach stays undone?
...
On return: the main loop verifies each finding in code+reality, discards the noise, and brings the survivors to the operator with concrete fixes. Accepted fixes get written into hub.md or a `feedback` memory — the audit's job is to keep the self-tuning honest.
